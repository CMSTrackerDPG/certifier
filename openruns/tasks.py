...
from dqmhelper.celery import app as celery_app
from django.contrib.auth import get_user_model

@celery_app.task()
def get_specific_open_runs(runs_list, username):
    runs = runregistry.get_runs(filter={
        'run_number': {'or': runs_list},
    })

    get_datasets_of_runs.delay(runs, username)

@celery_app.task()
def get_open_runs(start, end, username):
    runs = runregistry.get_runs(filter={
...
@celery_app.task()
def get_datasets_of_runs(runs, username):
