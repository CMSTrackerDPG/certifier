...
class ShiftLeaderTrackerCertificationFilter(django_filters.FilterSet):
    ...
    runreconstruction__run__b_field = django_filters.ModelChoiceFilter(
        queryset=OmsRun.objects.values_list("b_field", flat=True).distinct(),
        widget=forms.Select(attrs={
            'class': 'form-control',
        }))

    runreconstruction__run__fill__fill_type_runtime = django_filters.ModelChoiceFilter(
        queryset=OmsFill.objects.values_list("fill_type_runtime",
                                             flat=True).distinct(),
        widget=forms.Select(attrs={
            'class': 'form-control',
        }))

    runreconstruction__run__energy = django_filters.ModelChoiceFilter(
        queryset=OmsRun.objects.values_list("energy", flat=True).distinct(),
        widget=forms.Select(attrs={
            'class': 'form-control',
        }))

    runreconstruction__run__run_type = django_filters.ModelChoiceFilter(
        queryset=OmsRun.objects.values_list("run_type", flat=True).distinct(),
        widget=forms.Select(attrs={
            'class': 'form-control',
        }))

    runreconstruction__run__run_number__in = InFilter(
        field_name='runreconstruction__run__run_number', lookup_expr='in')
