...
def retrieve_fill(fill_number):
    ...
    if fill_check.exists():
        ...
        include_attribute_keys = [
            'fill_number', 'b_field', 'beta_star', 'bunches_beam1',
            'bunches_beam2', 'bunches_colliding', 'bunches_target',
            'crossing_angle', 'delivered_lumi', 'downtime', 'duration',
            'efficiency_lumi', 'efficiency_time', 'energy', 'era',
            'fill_type_party1', 'fill_type_party2', 'fill_type_runtime',
            'init_lumi', 'injection_scheme', 'intensity_beam1',
            'intensity_beam2', 'peak_lumi', 'peak_pileup',
            'peak_specific_lumi', 'recorded_lumi', 'first_run_number',
            'last_run_number'
        ]
        include_meta_keys = [
            'init_lumi', 'peak_lumi', 'delivered_lumi', 'recorded_lumi',
            'intensity_beam2', 'intensity_beam1', 'crossing_angle',
            'peak_specific_lumi', 'beta_star'
        ]

        fill_kwargs = {}
        for attribute_key in include_attribute_keys:
            if attribute_key in response['attributes'].keys():
                fill_kwargs[attribute_key] = response['attributes'][
                    attribute_key]
            ...
            if 'meta' in response.keys():
                if meta_key in response['meta']['row'].keys():
                    if response['meta']['row'][meta_key]['units']:
                        fill_kwargs[meta_key_unit] = response['meta']['row'][
                            meta_key]['units']
        ...
        try:
            ...
            OmsFill.objects.filter(fill_number=fill_number).update(
                **fill_kwargs)
...
def retrieve_run(run_number):
    ...
    if run_check.exists():
        ...
        include_attribute_keys = [
            "run_number", "b_field", "clock_type", "cmssw_version",
            "components", "delivered_lumi", "duration", "end_lumi", "energy",
            "fill_type_party1", "fill_type_party2", "fill_type_runtime",
            "hlt_key", "hlt_physics_counter", "hlt_physics_rate",
            "hlt_physics_size", "hlt_physics_throughput", "init_lumi",
            "initial_prescale_index", "l1_hlt_mode", "l1_hlt_mode_stripped",
            "l1_key", "l1_key_stripped", "l1_menu", "l1_rate",
            "l1_triggers_counter", "recorded_lumi", "sequence", "stable_beam",
            "tier0_transfer", "trigger_mode"
        ]
        include_meta_keys = [
            "init_lumi", "end_lumi", "delivered_lumi", "recorded_lumi"
        ]

        run_kwargs = {}
        for attribute_key in include_attribute_keys:
            if attribute_key in response['attributes'].keys():
                run_kwargs[attribute_key] = response['attributes'][
                    attribute_key]
        ...
        for meta_key in include_meta_keys:
            ...
            if 'meta' in response.keys():
                if meta_key in response['meta']['row'].keys():
                    if response['meta']['row'][meta_key]['units']:
                        run_kwargs[meta_key_unit] = response['meta']['row'][
                            meta_key]['units']
