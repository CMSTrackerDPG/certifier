...
def run_tracker_maps(run_type: str, run_number_list: list) -> bool:
    ...
    try:
        ...
        raise
    send_channel_message(channel_layer, "output_group", "-------- CONNECTED --------\n")
    send_channel_message(
...
def maps(request):
    ...
    if (
        ...
        logger.info(f"Got trackermaps generation request for {run_type}: {runs_list}")
        if runs_list:
            try:
                runs_list = list(
                    map(
        ...
        threading.Thread(target=run_tracker_maps, args=(run_type, runs_list)).start()
