...
def get_referenceruns():
    try:
       ...
       postgreSQL_select_Query = "select reference_run, lower(reco) from certhelper_referencerun"
       cursor.execute(postgreSQL_select_Query)
       mobile_records = cursor.fetchall()

       for row in mobile_records:
           try:
               run = retrieve_run(row[0])
               if not RunReconstruction.objects.filter(run__run_number=row[0],reconstruction=row[1]).exists():
                   RunReconstruction.objects.create(run=run, reconstruction=row[1], is_reference=True)
           except IndexError:
               pass
    ...
    try:
       ...
       for row in mobile_records:
           if not get_user_model().objects.filter(username=row[4]).exists():
               user = get_user_model().objects.create(id=int(row[0]), password=row[1] , last_login=row[2] , is_superuser=row[3] , username=row[4] , first_name=row[5] , last_name=row[6] , email=row[7] , is_staff=row[8] , is_active=row[9] , date_joined=row[10] , extra_data=row[11])
...
def get_trackercertification():
    try:
        ...
        for row in mobile_records:
            try:
                run = RunReconstruction.objects.get(run__run_number=row[0], reconstruction=row[1])
                ref_run = RunReconstruction.objects.get(run__run_number=row[8], reconstruction=row[9])
                ...
                if not TrackerCertification.objects.filter(runreconstruction=run).exists():
                    trackercertification = TrackerCertification.objects.create(user=user, runreconstruction=run, dataset=dataset,reference_runreconstruction=ref_run, trackermap=row[2], pixel=row[3], strip=row[4], tracking=row[5], pixel_lowstat=row[14], strip_lowstat=row[15], tracking_lowstat=row[16], date=row[7], comment=row[6])
